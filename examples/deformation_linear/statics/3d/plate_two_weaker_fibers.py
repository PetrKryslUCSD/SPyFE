...
SI = phunits.SI()
e1, e2, e3 = 20.0e6*SI.psi, 20.0e6*SI.psi, 2.0e6*SI.psi
...
def elem_dep_data_qt10ms(na, nb, nt):
    fens, fes = t4_block(a, b, t, na, nb, nt, orientation='ca')
    fens, fes = t4_to_t10(fens, fes)
...
for key, elem_dep_data  in models.items():
    print(key)
    for n  in range(3,4):
        fens, femm, sfemm = elem_dep_data(n * na, n * nb, n * nt)
        model_data = {}
        ...
        model_data['postprocessing']={'file': 'plate_two_weaker_fibers_' + key + '_results',
                                      'outcs': CSys()}
